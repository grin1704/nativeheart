...
export class EmailService {
   ...
   */
  async sendCollaboratorInvitation(data: CollaboratorInvitationEmailData): Promise<void> {
  ...
  /**
   * Sends notification when collaborator accepts invitation
   */
  async sendCollaboratorAcceptedNotification(data: CollaboratorAcceptedEmailData): Promise<void> {
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  /**
   * Sends notification when collaborator declines invitation
   */
  async sendCollaboratorDeclinedNotification(data: CollaboratorDeclinedEmailData): Promise<void> {
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  /**
   * Sends notification about page changes
   */
  async sendPageChangeNotification(data: PageChangeNotificationData): Promise<void> {
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
