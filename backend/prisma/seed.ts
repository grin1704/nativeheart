...
async function main() {
  ...
  // Create admin users
  const adminPassword = await bcrypt.hash('MemorialAdmin2024!@#', 10);
...
Он ушел от нас 20 августа 2023 года, оставив после себя любящую семью и множество добрых воспоминаний.`,
  ...
  // Create memory photos connections
  await prismaClient.memoryPhoto.create({
    data: {
      memoryId: memory1.id,
      mediaFileId: memoryPhoto1.id,
      orderIndex: 0
    }
  });
  ...
  // Create some content moderation entries
  ...
  console.log('\n🔑 Login credentials:');
  console.log('Admin: admin@memorial-pages.ru / MemorialAdmin2024!@#');
  console.log('Moderator: moderator@memorial-pages.ru / MemorialAdmin2024!@#');
